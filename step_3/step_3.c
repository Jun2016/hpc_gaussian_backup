...
* @param    t    	column t
*/
bool isInCoK(int k, int p1, int p2, int t){//p=p1*p2
...
/**
* @description      this funtion executes Gaussian elimination with total cyclic data distribution 
* @param    a   	matrix A
* @param    b    	matrix b
* @param    p1    	the number of processors, column side
* @param    p2    	the number of processors, row side
...
double * gauss_double_cyclic(double *a, double *b, int p1, int p2, int p, int me){
	...
	for ( k = 0; k < N-1 ; ++k)
	{
		...
		if (isInCoK(k, p1, p2, me))
		{
			...
			/* get the maximum |a_rk| with its processor's rank in y among column group */
		...
		if ((k%p2 == r%p2))//k,r same Ro
		{
			/* if processor me is in the same row group of k */
			if (isInRor(k, p1, p2, me))
			{
				if (r!=k)
				{
					/* if r is not the same as k, exchanging row r and k in matrix A and b locally */
					exchange_row_loc(a, b, r, k, p1, p2, me);
				}
				/* copying row k in matrix A and b into buf locally */
				copy_row_loc(a, b, k, buf, me, p1);
			}
		}
		else{/* pivot row and row k are owned by different processors */
			/* if current processor contains a part of the row k */
			if (isInRor(k, p1, p2, me))
			{
				/* copy row k into buf and send to the communication partner */
				copy_row_loc(a, b, k, buf, me, p1);
				q = compute_partner(Ro(r, p1, p2), me, p1);
				psz = compute_size(k);
				MPI_Send(buf+k, psz, MPI_DOUBLE, q, tag, MPI_COMM_WORLD);
			}
			else if (isInRor(r, p1, p2, me))/*if current processor contains a part of the pivot row*/
			{
				/* receive row k from communication partner and exchange row r with buf */
		...
		/* if processor has part of row k but doesn't have row r, writting received pivot row into row k */
	...
	for (k=N-1; k>=0; k--)
	{
		...
		if (isInRor(k, p1, p2, me))
		{
			...
			/* do MPI_SUM to sum the value of sum into sum_g in each processor in row group to which current processor belongs */
			MPI_Comm comm3;
			MPI_Group group3;
...
int main(int argc, char *argv[])
{
    ...
    /* p1 and p2 are mandatory in step_3 */
    if ( p1==-1 || p2 ==-1 )
    {
    	if(myrank==MASTER)printf("ERROR: requiring p1xp2\n");
		return 1;
    }
    ...
    t1 = MPI_Wtime();
    /* Executing Gaussian elimination with total cyclic data distribution */
